...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CuriumnMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CuriumnMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CuriumnMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("curiumn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1526939990 || vNodes.empty() || pwallet->IsLocked() || !pwallet->MintableCoins() || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced()) {
        ...
        LogPrintf("Running CuriumnMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),

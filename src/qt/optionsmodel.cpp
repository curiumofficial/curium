...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCuriumnAmount"))
        settings.setValue("nAnonymizeCuriumnAmount", 1000);

    nAnonymizeCuriumnAmount = settings.value("nAnonymizeCuriumnAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCuriumnAmount"))
        SoftSetArg("-anonymizecuriumnamount", settings.value("nAnonymizeCuriumnAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in curiumn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCuriumnAmount:
            return QVariant(nAnonymizeCuriumnAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCuriumnAmount:
            nAnonymizeCuriumnAmount = value.toInt();
            settings.setValue("nAnonymizeCuriumnAmount", nAnonymizeCuriumnAmount);
            emit anonymizeCuriumnAmountChanged(nAnonymizeCuriumnAmount);
            break;
        case CoinControlFeatures:
